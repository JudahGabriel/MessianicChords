    ...
    public class ChordSubmissionService
    {
        ...
        private async Task<ChordSubmission> CreateCore(ChordSubmissionRequest request)
        {
            ...
            if (request.Attachments.Count > 10)
            {
                ...
                tooManyAttachmentsError.Data.Add("attachmentCount", request.Attachments.Count);
                throw tooManyAttachmentsError;
            }

            // No attachment can be > 10MB
            var tooLargeAttachment = request.Attachments.FirstOrDefault(a => a.Length > 10_000_000);
            ...
            submission.ApproveRejectKey = Guid.NewGuid().ToString();
            ...
            await emailService.SendChordSubmissionEmail(submission, existingChordSheet, submission.ApproveRejectKey);
        ...
        private async Task ApproveOrRejectCore(ChordSubmissionApproval decision, string token)
        {
            ...
            if (!string.Equals(token, submission.ApproveRejectKey, StringComparison.OrdinalIgnoreCase))
            {
                throw new ArgumentException("Wrong approve/reject key");
            }
        ...
        private async Task<List<TempAttachment>> UploadTempAttachments(ChordSubmissionRequest request)
        {
            // Save the attachments to a temp location.
            var savedAttachments = new List<TempAttachment>(request.Attachments.Count);
            foreach (var attachment in request.Attachments)
            {
                ...
                var tempAttachment = new TempAttachment(cdnFileName, untrustedFileName, attachmentUri);
